...
makegenelist <- function(genomename = 'mm10', save = TRUE){
  ...
  gene, distance to the nearest downstream gene, gene GC content, and gene exon coverage. 
...
calrate <- function(time1file = time1file, time2file = time2file,
  ...
  cutoff in time1file will be considerred. Default is 1.
  
  @return A list including a data.frame recording the transcription elongation rates inferred using 
...
mcalrate <- function(time1files = c(wt0file, ko0file),
  ...
  Calculate gene elongation rate for multiple pairs of Pro-seq data, or Gro-seq data, 
  using an LSS (least sum of square) based method.
  ...
  If the sample is sequenced using a single strand method, set it as 0.
  @param threads Number of threads to do the parallelization. Default is 1.
  @param mergerefs Whether merge the data of all time1 reference files together to one reference, 
  and then use it as a uniform reference for all time2files. Default is TRUE.
  @param mergecases Whether merge the data of all time2 case files together to one case file. 
  Default is FALSE.
  @param savegenenames For which genes their concrete rate inference results need to be saved, 
  including the binned ratio values between their data in timefile2 and 
  timefile1, the extended ratio values for specific bins, etc.
  ...
  @return A list with each of its element is a sub-list recording the transcription elongation rate 
  inferred for each pair of Pro-seq or Gro-seq data using the LSS method, as well as other 
...
intracompare <- function(inferres = allrateres, targetgenes = NULL, quantilenum = 4){
  "
  Divide genes into different quantile groups according to their rates inferred.
  
  Divide genes into different quantile groups according to their rates inferred using 
  the calrate function, or mcalrate function.
  
  @param inferres The result list generated by the calrate function or mcalrate function. 
  @param targetgenes Which genes need to be divided into quantile groups. If it is NULL 
  (The default value), all genes provided by the parameter inferres 
  will be analyzed. If its value is 'significant', only genes judged as 
  significant genes by calrate or mcalrate will be analyzed. While the 
  the value of this parameter can also be a vector with gene symbols as 
  elements and only genes coverred by this vector will be analyzed.
  @param quantilenum How many quantile groups need to be divied. Default value is 4. 
  
  @return A list with 2 data.frames as elements. One of them records the absolute elongation 
  rates of the genes (bp/min) and divide the genes into several quantile groups with 
  genes in quantile1 group having the lowest rates. The other data.frame records the 
  relative elongation rates of the genes (percent of gene body/min) and divide them 
  into quantile groups. If the value of parameter inferres is from mcalrate, the 
  absolute and relative rates of the genes are the mean of all the Pro-seq or Gro-seq 
  pairs.
  @export
  "
...
orgintercompareinput <- function(reslist = allrateres, targetgenes = NULL){
  "
  Organize the results calculated by the function mcalrate to a data.frame
  
  Organize the results calculated by the function mcalrate to a data.frame 
  with the 1st column as gene symbols and the other columns as gene elongation 
  rates inferred for each Pro-seq or Gro-seq pair. This data.frame can be used 
  as input for the function intercompare.
  
  @param reslist The result list generated by the function mcalrate.
  @param targetgenes Which genes need to be included in the output of this function. If 
  it is NULL (The default value), all genes provided by the parameter 
  reslist will be analyzed. If its value is 'significant', only genes 
  judged as significant genes by mcalrate will be included. While the 
  the value of this parameter can also be a vector with gene symbols as 
  elements and only genes coverred by this vector will be included. 
  
  @return A data.frame with the 1st column as gene symbols and the other columns as gene 
  elongation rates inferred for each Pro-seq or Gro-seq pair. This data.frame can 
  be used as input for the function intercompare.
  @export
  "
...
intercompare <- function(inferresmat = rates, groupnames){
  "
  Compare gene elongation rate difference for each gene between 2 different conditions. 
  
  Compare gene elongation rate difference for each gene between 2 different conditions. 
  For each condition, if the number of replicates is < 3, Fisher's test will be 
  used to calculate the p-values and adjusted p-values of the rate differences, while if 
  the number of replicates is >= 3, Wilcox test and limma will be used to calculate 2 sets 
  of p-values and adjusted p-values (Wilcox test based and limma based).
  
  @param inferresmat A data.frame recording the elongation rates for the genes in different 
  conditions. Can be generated using the function orgintercompareinput.
  @param groupnames A vector with the elements corresponding to the condition names of the 
  rate columns in the data.frame indicated by the parameter inferresmat. 
  
  @return A data.frame with one row for one gene and the columns are the average transcription 
  elongation rates for each condition (log2 transformed), the log2FC, the p-value and 
  adjusted p-value (adjusted by Benjamini & Hochberg method) of the difference.
  @export
  "
...
metaplot <- function(metafile = metawt0file,
  ...
  Generate metagene plots from a bam file, as well as other information including the FPM value of each bp 
  position in the metagene plot, FPKM value in specific TSS, TTS, or gene body regions of each gene, etc. 
  
  @param metafile The bam file need to generate metagene plots. Can be a string indicating the directory of 
  the file, or a GAlignmentPairs object, or a GRanges object from the original bam file. 
  @param targetgenefile The genes whose FPM values need to be merged together to generate a metagene plot. If 
  it is NULL, all the genes in the genome specified by the parameter genomename will be 
  analyzed. If provied by the user, columns named as chr, start, end, strand, and gene_id 
  are required. All genes should be longer than the maximum value of the parameters 
  tssradius, ttsradius, and genelencutoff.
  @param genomename Specify the genome of the genes to be analyzed, when the parameter targetgenefile is NULL.
  @param tssradius If want to plot the metagene in TSS region, set this parameter as a vector with each 
  element corresponding to a radius of the TSS region centerred around the TSS site. Then, 
  for each radius value, a metagene plot will be generated.
  @param ttsradius If want to plot the metagene in TTS region, set this parameter as a vector with each 
  element corresponding to a radius of the TTS region centerred around the TTS site. Then, 
  for each radius value, a metagene plot will be generated. 
  @param genebodylen If want to plot the metagene in gene body region, set this parameter as a specific 
  numeric value (bp). Because different genes have different gene body length, before 
  merge their FPM values to a metagene, their gene body length will be scaled to a uniform 
  length, which is set by this parameter genebodylen. Genes with a longer length will be 
  compressed to this gene length, while the ones with a shorter length will be exteneded. 
  @param label A string will be included in the title of the metagene plot to indicate the experimental 
  condition. 
  @param strandmethod The strand specific method used when preparing the sequencing library, can be 1 for 
  directional ligation method and 2 for dUTP method. If the sample is sequenced using 
  a single strand method, set it as 0.
  @param threads Number of threads to do the parallelization. Default is 1.
  @param savegenenames For which genes their concrete FPM value for each bp position need to be saved, or 
  plotted.
  @param plotgenenames Whether to plot the FPM value for each bp position for the genes provided by 
  the parameter savegenenames
  @param plotfig Whether plot the metagenes or not. Default value is TRUE. For the genes indicated by 
  savegenenames, only if both the paramters plotgenenames and plotfig are TRUE, they will 
  be plotted.
  @param genelencutoff The cutoff on gene length (bp). The default value is NULL, but if it is set, only 
  genes longer than this cutoff will be considerred.
  @param fpkmcutoff The cutoff on gene FPKM value. Only genes with an FPKM value greater than the cutoff 
  will be considerred. Default is 1. 
  
  @return Will generate several metagene plots as well as a list containing the information of the FPKM 
  values on specific regions for each gene, the concrete FPM value on each bp position for the 
  metagene, and the genes indicated by the parameter savegenenames, etc. 
  @export
  "
...
mmetaplot <- function(metafiles = c(metawt0file, metako0file),
  ...
  Generate metagene plots for multiple bam files, as well as other information including the FPM value of each bp 
  position in the metagene plots, FPKM value in specific TSS, TTS, or gene body regions of each gene, etc. 
  
  @param metafiles The bam files need to generate metagene plots. Should be a vector with elements as strings 
  indicating the directores of the files.
  @param targetgenefile The genes whose FPM values need to be merged together to generate metagene plots. If 
  it is NULL, all the genes in the genome specified by the parameter genomename will be 
  analyzed. If provied by the user, columns named as chr, start, end, strand, and gene_id 
  are required. All genes should be longer than the maximum value of the parameters 
  tssradius, ttsradius, and genelencutoff.
  ...
  length, which is set by this parameter genebodylen. Genes with a longer length will be 
  compressed to this gene length, while the ones with a shorter length will be exteneded. 
  @param labels A vector with elements as strings to be included in the titles of the metagene plots to 
  indicate the experimental conditions of the bam files. There should be no replicated elements 
  in this vector.
  @param strandmethod The strand specific method used when preparing the sequencing library, can be 1 for 
  directional ligation method and 2 for dUTP method. If the sample is sequenced using 
  a single strand method, set it as 0.
  ...
  @param plotgenenames Whether to plot the FPM value for each bp position for the genes provided by 
  the parameter savegenenames
  @param mergecases Whether merge the data of all bam files together to one, and then use it to generate 
  the metagene plots. Default is FALSE.
  @param genelencutoff The cutoff on gene length (bp). The default value is NULL, but if it is set, only 
  genes longer than this cutoff will be considerred.
  @param fpkmcutoff The cutoff on gene FPKM value. Only genes with an FPKM value greater than the cutoff 
  will be considerred. Default is 1. 
  
  @return Will generate several metagene plots as well as a list with several sub-lists containing the 
  information of the FPKM values on specific regions for each gene, the concrete FPM value on each 
  bp position for the metagene, and the genes indicated by the parameter savegenenames, etc. 
  @export
  "
...
calpauseidx <- function(bamfile = time1chrsfile,
                        ...
                        fpkmcutoff = 1){
  "
  Calculate Pause indeces for genes from a bam file, and plot the FPM value of each bp for specific genes. 
  
  Calculate Pause indeces for genes from a bam file (Pause index is the ratio of transcription Pol 2 
  signal density near a gene promoter to signal density within the gene body, such that higher Pause 
  indices reflect a greater enrichment of promoter-paused Pol II), and also plot the FPM value of each 
  bp for specific genes. 
  
  @param bamfile The bam file need to calculate the Pause indeces for genes. Can be a string indicating 
  the directory of the file, or a GAlignmentPairs object, or a GRanges object from the 
  original bam file. 
  @param genefile The genes whose Pause indeces need to be calculated. If it is NULL, all the genes in 
  the genome specified by the parameter genomename will be analyzed. If provied by the 
  user, columns named as chr, start, end, strand, and gene_id are required. All genes 
  should be longer than the maximum value of the parameters tssradius and genelencutoff.
  @param genomename Specify the genome of the genes to be analyzed, when the parameter genefile is NULL.
  @param tssradius A numeric value indicating the radius of the promoter region centerred around the TSS 
  @param label A string will be included in the titles of the specific gene plots to indicate the 
  ...
  @return Will generate a list containing a data.frame recording the Pause indeces of the genes, as well as 
  plot the FPM value of each bp for the specific genes indicated by the parameter savegenenames.
  @export
  "
...
mcalpauseidx <- function(bamfiles = c(time1chrsfile, time2chrsfile),
  ...
  "
  Calculate Pause indeces for genes for multiple bam files, and plot the FPM value of each bp for specific 
  genes. 
  
  Calculate Pause indeces for genes for mulitple bam files (Pause index is the ratio of transcription Pol 2 
  signal density near a gene promoter to signal density within the gene body, such that higher Pause indices 
  reflect a greater enrichment of promoter-paused Pol II), and also plot the FPM value of each bp for 
  specific genes. 
  
  @param bamfiles The bam files need to calculate the Pause indeces for genes. Should be a vector with elements 
  as strings indicating the directores of the files.
  @param genefile The genes whose Pause indeces need to be calculated. If it is NULL, all the genes in 
  the genome specified by the parameter genomename will be analyzed. If provied by the 
  user, columns named as chr, start, end, strand, and gene_id are required. All genes 
  should be longer than the maximum value of the parameters tssradius and genelencutoff.
  @param genomename Specify the genome of the genes to be analyzed, when the parameter genefile is NULL.
  @param tssradius A numeric value indicating the radius of the promoter region centerred around the TSS 
  @param labels A vector with elements as strings to be included in the titles of the specific gene plots 
  to indicate the experimental conditions of the bam files. There should be no replicated 
  ...
  @param plotgenenames Whether to plot the FPM value for each bp position for the genes provided by 
  the parameter savegenenames
  @param mergecases Whether merge the data of all bam files together to one, and then use it to calculate 
  gene Pause indeces and plot genes. Default is FALSE.
  @param threads Number of threads to do the parallelization. Default is 1.
  @param genelencutoff The cutoff on gene length (bp). The default value is NULL, but if it is set, only 
  genes longer than this cutoff will be considerred.
  @param fpkmcutoff The cutoff on gene FPKM value. Only genes with an FPKM value greater than the cutoff 
  ...
  @return Will generate a list with several sub-lists recording the Pause indeces of the genes, as well as 
  plot the FPM value of each bp for the specific genes indicated by the parameter savegenenames.
  @export
  "
...
getgc <- function(targetfile, genomename = 'mm10', genenames = NULL){
  ...
  @param targetfile The directory of the file indicating the gene regions whose GC contents need 
  to be calculated, not necessary to be the gene body region between TSS and 
  TTS sites. Columns named as chr, start, end, strand, and gene_id are required. 
  If it is NULL, all the gene body regions of UCSC known genes in the genome 
  specified by the parameter genomename will be analyzed.
  @param genomename Specify the genome of the genes to be analyzed, when the parameter targetfile 
  is NULL.
  @param genenames The symbols of genes whose GC contents need to be calculated. The regions indicated 
  by the parameter targetfile or genomename will be analyzed only if they belong to 
  the genes indicated by this parameter genenames. 
  
  @return A data.frame with a column named GC indicating the GC contents of the gene regions. 
  @export
  "
...
getkmer <- function(targetfile, genomename = 'mm10', k = 6,
  ...
  Compare the kmer difference between 2 sets of sequences, reporting the ratio between the 
  kmer frequencies of the 2 sequence sets, the p-value and adjusted p-value of the difference.
  
  @param targetfile The directory of the file indicating the gene regions whose kmer need 
  to be compared between the 2 sequence sets, not necessary to be the gene body 
  region between TSS and TTS sites. Columns named as chr, start, end, strand, 
  and gene_id are required. If it is NULL, all the gene body regions of UCSC 
  known genes in the genome specified by the parameter genomename will be analyzed.
  @param genomename Specify the genome of the genes to be analyzed, when the parameter targetfile 
  is NULL.
  @param k The length of the kmer to be analyzed. Default is 6, meaning 6-mer will be analyzed. 
  @param genes1 The symbols of genes in set 1 whose kmers need to be compared with that of set 2. 
  The regions indicated by the parameter targetfile or genomename will be used for 
  this set only if they belong to the genes indicated by this parameter genes1. 
  @param genes2 The symbols of genes in set 2 whose kmers need to be compared with that of set 1. 
  Similar to the parameter genes1
  @param feature If the parameter targetfile is NULL, while the parameter genomename is defined. 
  This parameter feature can be used to further select regions from the genes 
  indicated by genomename. Can choose from 'promoter', 'end', and 'genebody'. If 
  it is 'promoter' or 'end', another parameter radius is needed to define the radius 
  of the promoter or end region centerred the TSS or TTS site.
  @param radius A numberic value needed to define the radius length of the gene promoter or end region 
  if the parameter feature is set as 'promoter' or 'end'.
  
  @return A data.frame indicating the kmer frquency ratios between sequence set 1 and set 2, as well 
  as p-values (calculated with Fisher's test) and adjusted p-values (adjusted using the Benjamini 
  & Hochberg method)
  @export
  "
...
getexon <- function(targetfile, genomename = 'mm10', genenames = NULL){
  ...
  @param targetfile The directory of the file indicating the gene regions whose exon coverages need 
  to be calculated, not necessary to be the gene body region between TSS and 
  TTS sites. Columns named as chr, start, end, strand, and gene_id are required. 
  If it is NULL, all the gene body regions of UCSC known genes in the genome 
  specified by the parameter genomename will be analyzed.
  @param genomename Specify the genome of the genes to be analyzed, when the parameter targetfile 
  is NULL.
  @param genenames The symbols of genes whose exon coverages need to be calculated. The regions 
  indicated by the parameter targetfile or genomename will be analyzed only if 
  they belong to the genes indicated by this parameter genenames. 
  
  @return A data.frame with a column named exon indicating the exon coverages of the gene regions. 
  @export
  "
