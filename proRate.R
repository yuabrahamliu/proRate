...
wkdir <- 'C:/Users/uiluy/Desktop/projects/TianLab/liming/enlongation/elongHMM/'

setwd(wkdir)


time1file <- './elongHMM_pipeline/data/bd12Aligned.sortedByCoord.out.qc.sorted.chrX.bam'
time2file <- './elongHMM_pipeline/data/bd16Aligned.sortedByCoord.out.qc.sorted.chrX.bam'
targetfile <- './elongHMM_pipeline/data/uniq_target_packagewriting.txt'

time1chrsfile <- './elongHMM_pipeline/data/bd12Aligned.sortedByCoord.out.qc.sorted.chr19chrX.bam'
time2chrsfile <- './elongHMM_pipeline/data/bd16Aligned.sortedByCoord.out.qc.sorted.chr19chrX.bam'
targetchrsfile <- './elongHMM_pipeline/data/uniq_target_packagewriting.chr19chrX.txt'


wt0file <- time1file
wt15file <- time2file
wt30file <- './elongHMM_pipeline/data/bd18Aligned.sortedByCoord.out.qc.sorted.chrX.bam'

ko0file <- './elongHMM_pipeline/data/bd13Aligned.sortedByCoord.out.qc.sorted.chrX.bam'
ko15file <- './elongHMM_pipeline/data/bd17Aligned.sortedByCoord.out.qc.sorted.chrX.bam'
ko30file <- './elongHMM_pipeline/data/bd19Aligned.sortedByCoord.out.qc.sorted.chrX.bam'

metawt0file <- './ssmeta/bd12Aligned.sortedByCoord.out.qc.sorted.chrs.bam'
metako0file <- './ssmeta/bd13Aligned.sortedByCoord.out.qc.sorted.chrs.bam'
metagenefile <- './ssmeta/lnc.mm10.apa.sub.txt'


pauseidxtargetfile <- './elongHMM_pipeline/data/pauseidxgenes_packagewriting.chr19chrX.txt'
...
makegenelist <- function(genomename = 'mm10', save = TRUE){
  ...
  if(genomename == 'hg19'){
    ...
    genecoords <- genes(TxDb.Hsapiens.UCSC.hg19.knownGene)
...
getreadslist <- function(timefiles = time1files, mergefiles = mergerefs){
...
calrate <- function(time1file = time1file, time2file = time2file, 
  ...
  "
  @time1file - The reference Pro-seq/Gro-seq bam file name, corresponding to 0min.
               Can be a string indicating the directory of the file, or a GAlignmentPairs object, 
               or a GRanges object from the original bam file
  @time2file - The treatment Pro-seq/Gro-seq bam file name, corresponding to a specific time point 
               (e.g. 15min if DRB was used for 15min). 
               Can be a string indicating the directory of the file, or a GAlignmentPairs object, 
               or a GRanges obejct from the original bam file
  @targetfile - The genes whose transcriptional elongation rate need to be analyzed. If it is NULL, all the 
                genes longer than 4kb in the genome specified by the parameter genomename will be 
                analyzed.
                If provied by the user, columns named as chr, start, end, strand, and gene_id are required. 
                All genes should have a length greater than 4kb.
  @genomename - Specify the genome of the genes to be analyzed, when the parameter targetfile is NULL. 
  @time - The time difference between time1file and time2file, using min as its unit.
  @strandmethod - The strand specific method used when preparing the sequencing library, 
  ...
  @drugtype - The drug used to pretreat the cells to test the transcription rate. If it is DRB, FP, or Trp, 
              set it as 1, if it is E2, set it as 2.
...
mcalrate <- function(time1files = c(wt0file, ko0file), 
  ...
  "
  @time1files - The reference Pro-seq/Gro-seq bam file names, corresponding to 0min.
                Should be a vector with elements as strings indicating the directores of the files
  @time2files - The treatment Pro-seq/Gro-seq bam file name, corresponding to a specific time point 
                (e.g. 15min if DRB was used for 15min). 
                Should be a vector with elements as strings indicating the directores of the files
  @targetfile - The genes whose transcriptional elongation rate need to be analyzed. If it is NULL, all the 
                genes longer than 4kb in the genome specified by the parameter genomename will be 
  ...
  time1files <- getreadslist(timefiles = time1files, mergefiles = mergerefs)
  time2files <- getreadslist(timefiles = time2files, mergefiles = mergecases)
  ...
  for(i in 1:timefilelength){
    ...
    res <- calrate(time1file = time1file, time2file = time2file, 
                   ...
                   time = time, 
                   strandmethod = strandmethod, drugtype = drugtype, 
                   threads = threads, 
...
bootmean <- function(randomseed = 1, ratesres = rates){
  
  row.names(ratesres) <- 1:nrow(ratesres)
  set.seed(randomseed)
  
  idx <- sample(x = 1:nrow(ratesres), size = nrow(ratesres), replace = TRUE)
  bootres <- ratesres[idx,]
  
  bootstats <- data.frame(boot = randomseed, absmean = mean(bootres$absrate), relmean = mean(bootres$relrate), 
                          stringsAsFactors = FALSE)
...
wilcoxcomp <- function(idx, mat1 = dat1, mat2 = dat2){
  ...
  wilcoxres <- wilcox.test(mat1[idx,], mat2[idx,])
...
intracompare <- function(inferres = tst1thread, targetgenes = NULL, threads = 1, bootnum = 1000){
  
  if((!is.data.frame(inferres)) & (length(inferres) > 1)){
  ...
  randomseeds <- 1:bootnum
  randomseeds <- as.list(randomseeds)
  
  bootmeans <- lapply(X = randomseeds, FUN = bootmean, ratesres = rates)
  bootmeans <- do.call(rbind, bootmeans)
  bootabs <- bootmeans$absmean[order(bootmeans$absmean)]
  bootrel <- bootmeans$relmean[order(bootmeans$relmean)]
  
  abspercentile <- ecdf(x = bootabs)
  relpercentile <- ecdf(x = bootrel)
  
  abspvals <- abspercentile(rates$absrate)
  relpvals <- relpercentile(rates$relrate)
  
  absmean <- mean(rates$absrate)
  relmean <- mean(rates$relrate)
  
  revabspvals <- 1 - abspvals
  revrelpvals <- 1 - relpvals
  
  absps <- as.vector(rbind(abspvals, revabspvals))[as.vector(rbind(abspvals, revabspvals)) < 0.5]
  relps <- as.vector(rbind(relpvals, revrelpvals))[as.vector(rbind(relpvals, revrelpvals)) < 0.5]
  
  abspadjs <- p.adjust(absps, method = 'BH')
  relpadjs <- p.adjust(relps, method = 'BH')
  
  absres <- data.frame(gene_id = rates$gene_id, absrate = rates$absrate, 
                       pval = absps, padj = abspadjs, log2FC = log2(rates$absrate/absmean), 
                       stringsAsFactors = FALSE)
  relres <- data.frame(gene_id = rates$gene_id, relrate = rates$relrate, 
                       pval = relps, padj = relpadjs, log2FC = log2(rates$relrate/relmean), 
                       stringsAsFactors = FALSE)
  
  absres <- absres[order(absres$padj, absres$pval, -abs(absres$log2FC)),]
  relres <- relres[order(relres$padj, relres$pval, -abs(relres$log2FC)),]
  
  row.names(absres) <- 1:nrow(absres)
  row.names(relres) <- 1:nrow(relres)
  
  res <- list(absolute_rate = absres, relative_rate = relres)
  
  return(res)
  
}

orgintercompareinput <- function(reslist = reslist){
  ...
  names(reses) <- c('gene_id', paste0('rate_', seq(1, (ncol(rates) - 1), 1)))
...
intercompare <- function(inferresmat = rates, groupnames = c('15min', '30min')){
  ...
  if(ncol(dat1) < 3 & ncol(dat2) < 3){
    ...
    pvals <- unlist(pvals)
    padjs <- p.adjust(pvals, method = 'BH')
    log2fcs <- log2(rowMeans(dat2)/rowMeans(dat1))
  ...
  res <- res[order(res$padj, res$pval, -abs(res$log2FC)),]
...
#ssMetagene (ssMetagene and pausing index)#####
...
mmetaplot <- function(metafiles = c(metawt0file, metako0file), 
  ...
  metafiles <- getreadslist(timefiles = metafiles, mergefiles = mergecases)
...
mcalpauseidx <- function(bamfiles = c(time1chrsfile, time2chrsfile), 
  ...
  bamfiles <- getreadslist(timefiles = bamfiles, mergefiles = mergecases)
...
tst1thread <- calrate(time1file = time1file, time2file = time2file, 
                      targetfile = targetfile, 
                      time = 15, 
                      strandmethod = 1, drugtype = 1, 
                      threads = 4)
...
tst4thread <- calrate(time1file = time1file, time2file = time2file, 
                      targetfile = targetfile, 
                      time = 15, 
                      strandmethod = 1, drugtype = 1, 
                      threads = 4)

reslist <- mcalrate(time1files = c(wt0file, ko0file), 
                    time2files = c(wt15file, wt30file), 
                    targetfile = targetfile, 
                    genomename = NULL, 
                    times = c(15, 30), 
...
intracompareres <- intracompare(inferres = reslist, threads = 1)

rates <- orgintercompareinput(reslist = reslist)

intercompareres <- intercompare(inferresmat = rates, groupnames = c('15min', '30min'))


metareslit <- mmetaplot(metafiles = c(metawt0file, metako0file), 
                        targetgenefile = metagenefile, 
                        genomename = NULL, 
                        tssradius = c(2000, 1000, 500), ttsradius = c(2000, 1000, 500), 
                        genebodylen = 4000,
...
pauselist <- mcalpauseidx(bamfiles = c(time1chrsfile, time2chrsfile), genefile = pauseidxtargetfile, 
                          genomename = NULL, tssradius = 1000, labels = c('0min', '15min'), 
                          strandmethod = 1, savegenenames = c('Nolc1', 'Lamp2', 'Pdk3'), plotgenenames = TRUE, 
                          mergecases = FALSE, 
                          threads = 4)
